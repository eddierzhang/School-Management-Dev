...
def watchlist(
    ...
    rows = [s for s in sigs.values() if s.band in bands]
...
def strengths(
    ...
    """
    rows = [s for s in sigs.values() if s.excel_index >= 60]
...
def gaps(
    ...
    return [SkillGapOut.model_validate(g) for g in skill_gaps(db)[:limit]]
