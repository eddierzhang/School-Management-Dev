...
Students, courses and enrollments come from `../demo/seed/` — the same JSON the
registrar console is seeded from, generated by `node demo/gen_seed.js` in the repo
...
SEED_DIR = Path(os.environ.get("HR_SEED_DIR") or Path(__file__).parent.parent / "demo" / "seed")
...
def load_console_seed() -> tuple[list[dict], list[dict], list[dict]]:
    if not SEED_DIR.exists():
        sys.exit(f"No {SEED_DIR} — run `node demo/gen_seed.js` first.")
    ...
    if not students or not courses:
        sys.exit(f"{SEED_DIR} is missing students or courses — re-run `node demo/gen_seed.js`.")
