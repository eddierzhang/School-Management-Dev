...
import type { AgentRun, ProposalRow, TranscriptStep } from '../types'
import { ErrorNote, Icon, Loading, Pill } from '../components/ui'
      ...
      {steps.map((s) => (
        <div key={s.step} style={{ borderLeft: '2px solid var(--rule)', paddingLeft: 11 }}>
          <div className="sub" style={{ fontWeight: 600 }}>
            {s.step === 0 ? 'Opening read (automatic)' : `Step ${s.step}`}
            {s.ms > 0 && <> · {secs(s.ms)}</>}
...
export function Agents() {
  ...
  // Runs take minutes on local hardware, so poll while anything is in flight.
  useEffect(() => {
  ...
  async function start(name: string) {
    ...
    try {
      ...
      setNotice(`${name} started. Local inference takes a few minutes — this page updates itself.`)
      await refresh()
    } catch (e) {
  ...
  async function decide(p: ProposalRow, approve: boolean) {
  ...
  if (fleet.loading) return <Loading what="the fleet" />
  if (fleet.error) return <ErrorNote error={fleet.error} onRetry={fleet.reload} />
    ...
    <>
      <section className="sec">
        <div className="sec-head">
          ...
          <span className="sub">running locally on {rt.model}</span>
        </div>
        ...
        {rt.reachable && !rt.can_run_agents && (
          <div className="banner critical">
            <Icon name="critical" />
            <span style={{ flex: 1, minWidth: 240 }}>
        ...
        <div className="cards">
          {fleet.data.agents.map((a) => {
            const running = runs.find((r) => r.agent === a.name && r.status === 'running')
            return (
              <article className="icard" key={a.name}>
                ...
                <div className="field">
                  ...
                  <textarea
                    id={`task-${a.name}`} className="inp" rows={2}
                ...
                <div style={{ marginTop: 'auto', paddingTop: 4 }}>
                  <button className="btn primary" disabled={!rt.can_run_agents || busy === a.name || !!running}
                    onClick={() => start(a.name)}>
                    {running ? 'Running…' : busy === a.name ? 'Starting…' : 'Run agent'}
                  </button>
                </div>
              </article>
            )
      ...
      <section className="sec">
        <div className="sec-head">
          ...
          <span className="spacer" />
          <span className="sub">{proposals.length} pending</span>
        </div>
        <p className="sec-note">
          ...
          <div className="panelbox queue">
            {proposals.map((p) => (
              <div className="qrow" key={p.id} style={{ ['--sev' as string]: 'var(--signal)' }}>
                ...
                <button className="btn sm primary" onClick={() => decide(p, true)}>Approve</button>
                <button className="btn sm ghost" onClick={() => decide(p, false)}>Reject</button>
              </div>
            ))}
          </div>
        )}
      ...
      <section className="sec">
        ...
        {runs.length === 0 ? (
          <div className="panelbox"><div className="qempty">No agent has run yet.</div></div>
        ) : (
              ...
              <tbody>
                {runs.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td className="nowrap">
                      <button className="btn sm" onClick={async () => setOpenRun(await api.run(r.id))}>
                        Transcript
                      </button>
                    </td>
                  </tr>
      ...
      {openRun && (
        <>
          <button className="scrim" onClick={() => setOpenRun(null)} aria-label="Close transcript" />
          <aside className="drawer" role="dialog" aria-modal="true" aria-labelledby="run-title">
            <div className="drawer-head">
              <div style={{ flex: 1, minWidth: 0 }}>
                <h2 id="run-title">Run #{openRun.id} — {openRun.agent}</h2>
                <div className="sub" style={{ marginTop: 3 }}>
                  {openRun.model} · {openRun.steps_used} steps · {secs(openRun.duration_ms)}
                  {openRun.tool_errors > 0 && <> · {openRun.tool_errors} rejected call(s)</>}
                </div>
              </div>
              <button className="btn sm ghost" onClick={() => setOpenRun(null)}>Close</button>
            </div>
            <div className="drawer-body">
              <div className="block">
                <h3>Task</h3>
                <p style={{ margin: 0, fontSize: 12.5 }}>{openRun.prompt}</p>
              </div>
              {openRun.error && <ErrorNote error={openRun.error} />}
              {openRun.summary && (
                <div className="block">
                  <h3>What it concluded</h3>
                  <p style={{ margin: 0, fontSize: 12.5, whiteSpace: 'pre-wrap' }}>{openRun.summary.slice(0, 1500)}</p>
                </div>
              )}
              <div className="block">
                <h3>Every step it took</h3>
                <Transcript steps={openRun.transcript ?? []} />
              </div>
            </div>
          </aside>
        </>
